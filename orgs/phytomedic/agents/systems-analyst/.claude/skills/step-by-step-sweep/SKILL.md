...
## Phase 0: Auth Surface Coverage Check (run at the START of every sweep batch)

Before testing any pages, check for new routes added since the last sweep that may lack middleware protection.

```bash
# Find Next.js page files added since last sweep (adjust date to last sweep date)
git -C /Users/arndt/phytomedic-saas log --since="7 days ago" --name-only --diff-filter=A \
  --pretty=format: -- "src/app/**/*.tsx" "src/app/**/*.ts" | grep -E "page\.(tsx|ts)$" | sort -u

# Then verify each new route appears in proxy.ts matcher OR is explicitly marked public
grep -n "matcher\|publicRoutes\|isPublicRoute" /Users/arndt/phytomedic-saas/src/middleware.ts \
  /Users/arndt/phytomedic-saas/src/proxy.ts 2>/dev/null | head -30
```

**Rule:** For each new page route found:
- If it is a protected route (patient data, portal, onboarding sub-step) and NOT in the matcher → file a HIGH security task immediately, route to backend-architect or frontend-dev.
- If it is intentionally public → document it explicitly in this section as "confirmed public: /path".
- Do NOT skip this phase. The /onboarding/arzt + /onboarding/apotheke auth bypass (2026-04-29) was caught because of a sweep, but it should have been caught at route creation time.

**Confirmed intentionally public routes (do not flag):**
- /onboarding (role selection hub)
- /medizin/* (all public medical content)
- /wissen/*, /krankheiten/*, /freizeit/* (public content)
- /agb, /datenschutz, /impressum, /widerruf (legal)
- /registrieren, /login (auth entry points)
